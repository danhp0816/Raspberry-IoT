...
def generar_grafica(DB_PATH, fecha=None, inicio="00:00:00", fin="23:59:59"):
    # 1. Conectar y extraer datos

    inicio_completo = f"{fecha} {inicio}"
    fin_completo = f"{fecha} {fin}"
    
    conn = sqlite3.connect(DB_PATH)
    query = """
        SELECT fecha, temperatura
        FROM lecturas 
        WHERE fecha BETWEEN '{inicio_completo}' AND '{fin_completo}'
        ORDER BY fecha ASC
    """
    ...
    plt.tight_layout()
    plt.savefig(f'reporte_{fecha}_{inicio}h.png')
